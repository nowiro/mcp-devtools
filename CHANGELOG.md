...
- **`.github/prompts/clarify.prompt.md`** — slash-command `/clarify` (`mode: agent`): domyka placeholdery `[?]` w `docs/specs/<slug>/spec.md` przez ustrukturyzowane Q&A, dopisuje sekcję `## Clarifications`, flip `status: draft → clarified` gdy brak `[?]`, re-walidacja przez `npm run sdd:check`. Środkowy krok triady SDD (`/new-tool → /clarify → /analyze → /implement`). Read-only poza docelowym `spec.md`; nie zmyśla (unanswered → zostaje `draft`).
- **`.github/prompts/analyze.prompt.md`** — slash-command `/analyze` (`mode: agent`): cross-artifact SDD consistency check (spec↔plan↔kod) przed `/implement`. Inspiracja spec-kit `/speckit.analyze`. Najpierw odpala deterministyczny `sdd:check`, potem warstwa semantyczna (pokrycie AC, dryf kodu, sprzeczności).
- **`tools/scripts/validate-sdd.mjs`** + **`sdd:check`** (wpięty w `verify`) — deterministyczna walidacja strukturalna `docs/specs/` + `docs/plans/`: frontmatter (`type`/`id`), wymagane sekcje specu, tabela zadań planu (`id | title | agent | done_when`), traceability `plan.new-tool.<slug>` → `spec.md`, oraz reguła „non-draft spec nie może mieć `[?]`". Przechodzi „na pusto" bez artefaktów.
- **`.github/skills/propose-fix/SKILL.md`** + **`.github/skills/playwright-sanity/SKILL.md`** — dwa nowe Agent Skills (read-first, sandbox `PROJECT_ROOT`): `propose-fix` (pakiet kontekstu pod `propose_fix` + minimalna poprawka), `playwright-sanity` (zawężony run e2e przez `run_playwright`, reporter json/junit).
- **`tools/scripts/validate-ai-config.mjs` — reguła „description = routing rule".** Sekcja 6 wymaga, by opis skilla zawierał klauzulę-trigger („Use when …") i miał ≥ 40 znaków (konwencja skills.sh), zamiast być samym tytułem.
- **`.github/workflows/ci.yml`** — `npm run verify` na `pull_request` + push do `main`, matryca OS (ubuntu / windows / macos) dowodząca cross-platform determinism; `concurrency` + `cache: npm`.
